...
const sendRedirectFile = module.exports.sendRedirectFile = (fixedDirectory, res) => {
...
const ifTsFilesAreReadyThenSend = 
  module.exports.ifTsFilesAreReadyThenSend = ({config, address, res, index = 0}) => {
  dirUtils
      ...
      .then( (ready) => {
        if (ready) {
          ...
          if (index < RETRY_MAX) {
            setTimeout(() => {
              ifTsFilesAreReadyThenSend({config, address, res, index: index + 1});
            }, 1000);
            w(o.holding());
          } else {
